...
def hent_data_fra_github():
    """Henter de ferdige CSV-resultatene fra GitHub Releases."""
    try:
        df = pd.read_csv(GITHUB_RAW_URL)
        # Sorterer dataen på nytt, bare for å være sikker
        df = df.sort_values(by='total_score', ascending=False).reset_index(drop=True)
        # Formaterer visningskolonner
...
if df_results.empty:
    ...
    with st.sidebar:
        ...
        sektor_valg = ['Alle Sektorer'] + sorted(df_results['sector'].unique().tolist())
    ...
    st.dataframe(
        ...
        column_order=['ticker', 'name', 'sector', 'total_score', 'V/B Score', 'Momentum Score', 'Volum Score', 'Optimal SL (%)', 'CAGR Test (%)', 'Max DD Test (%)', 'why_selected'],
        column_config={
            "ticker": "Ticker",
            "name": "Selskapsnavn",
            "sector": "Sektor",
            "total_score": st.column_config.NumberColumn("Total Score", help="Vektet Score (50% V/B + 30% Mom + 20% Vol)"),
            "why_selected": st.column_config.TextColumn("Detaljer"),
            "Optimal SL (%)": st.column_config.NumberColumn("Optimal SL (Train)", help="SL-prosent optimalisert på Train-settet"),
        }
    )
